...
[[ "$(config_value_or wireguard client_mode management)" == management ]]
[[ "$(wireguard_client_allowed_ips)" == 10.66.66.0/24 ]]
...
sed 's/client_mode: management/client_mode: invalid/' "$tmp/good.yaml" > "$tmp/bad-client-mode.yaml"
...
sed 's/client_mode: management/client_mode: full/' "$tmp/good.yaml" > "$tmp/full.yaml"
CONFIG_FILE="$tmp/full.yaml"; validate_config; [[ "$(wireguard_client_allowed_ips)" == 0.0.0.0/0 ]]
