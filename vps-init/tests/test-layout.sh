...
for file in install.sh doctor.sh wg-add-client.sh wg-remove-client.sh lib/common.sh lib/config.sh config/vps.example.yaml scripts/preflight.sh scripts/base.sh scripts/firewall.sh scripts/ssh-hardening.sh scripts/wireguard.sh scripts/wg-add-client.sh scripts/wg-remove-client.sh scripts/sing-box.sh scripts/generate-clash-profile.sh; do [[ -f "$ROOT/$file" ]]; done
